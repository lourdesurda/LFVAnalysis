...
void CMSAnalysis::AddDataSampleFiles(const TString& id,  const TString& dir, const TString& file, double luminosity, int nfiles) 
{
	...
	//SAMPLES::TotalAnalysisLumi;
        ...
        _sampleXsection.push_back(_sampleNevents.back()/luminosity);////PARECE QUE NO SE HACE NADA CON ELLA AQUI EN ESTE .C
        _sampleWeight.push_back(1.);////SUSITITUIDA Y NO COMETNADA (YA EN FUNCIONAMIENTO)
        _signalFlag.push_back(false); // set to false for data////SUSTITUIDA YA
...
TH2D* CMSAnalysis::ReadingFileAndGettingTH2Histogram(TString path, TString histname)
{
	TFile *f = TFile::Open(path);
...
void CMSAnalysis::SetTree(int i) 
{
      ...
      printf("Processing sample '%s'...\n", _sampleFile[i].Data());
	//// PARA SUSTITUIR POR LA LINEA DE ARRIBA printf("Processing sample '%s'...\n", sample.SampleFile[i].Data());
      _currentIndex = i;
      _file = new TFile(_sampleFile[i],"READONLY");
	////PARA SUSTITUIR POR LA LINEA DE ARRIBA _file = new TFile(sample.SampleFile[i],"READONLY");
      _NANOTREE = NULL;
      _file->GetObject("Events",_NANOTREE);
...
/*void CMSAnalysis::DrawPlot1D(const TString& name, const TString& suffix, const TString& dir) //name of histogram
{
      	...
      	for (unsigned int j=0; j<nhists; ++j) 
	{
		...
		////PARA SUSTITUIR POR LA LINEA DE ARRIBA for (unsigned int i=0; i<sample.SampleFile.size(); ++i)
		{
	...
	// Blinding of the data in the signal-dominated regions.
	...
	hMCStack->Draw("same hist F");
        ...
        TLatex* luminosity = new TLatex(0.6,0.95,Form("L = %.1f fb^{-1} (13 TeV)",SAMPLES::TotalAnalysisLumi/1000.));
	luminosity->SetNDC();
...
/*void CMSAnalysis::AddMCSignalSample(const TString& id, const TString& file, int maxevents, double xsec, int total_events_for_xsection) 
{
	...
	_SampleInfo[_SampleInfo.size()-1].SignalFlag = true;  // set signal flag to true
      //  _sampleNFiles.push_back(0);
}*/
